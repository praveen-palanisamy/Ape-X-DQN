...
class DuellingDQN(torch.nn.Module):
    def __init__(self, state_shape, action_dim):
        ...
        self.layer1 = torch.nn.Sequential(torch.nn.Conv2d(state_shape[0], 32, 8, stride=4),
                                          torch.nn.ReLU())
        ...
        self.layer3 = torch.nn.Sequential(torch.nn.Conv2d(64, 64, 3, stride=1),
                                          torch.nn.ReLU())
        self.value_stream_layer = torch.nn.Sequential(torch.nn.Linear(4 * 64, 512),
                                                      torch.nn.ReLU())
        self.advantage_stream_layer = torch.nn.Sequential(torch.nn.Linear(4 * 64, 512),
                                                          torch.nn.ReLU())
    ...
    def forward(self, x):
        ...
        x = self.layer1(self.layer2(self.layer3(x)))
        value = self.value(self.value_stream_layer(x))
        advantage = self.advantage(self.advantage_stream_layer)
